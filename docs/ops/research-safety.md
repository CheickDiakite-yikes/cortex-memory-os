...
| 2026-05-02 | Clicky UI/UX pass | farzaa/clicky repository at commit `a80fa80721a8aebe51a170a7780705024ebc6e46`: https://github.com/farzaa/clicky | 1 | Extracted Shadow Pointer, compact control panel, point-at-element, onboarding, and trust-surface lessons for Cortex | Primary repository inspected read-only. Setup commands, package installs, Xcode builds, API calls, and worker deployment were not executed. Findings recorded in `docs/research/clicky-ui-ux-analysis-2026-05-02.md`. |
| 2026-04-29 | Frontier research to Codex plugin packaging | `docs/research/frontier-agent-plugin-lessons-2026-04-29.md` plus official/primary sources above | 1 | Applied research lessons to `CODEX-PLUGIN-001` plugin skeleton, MCP config, progressive-disclosure skills, and safe execution refs | External instructions stayed untrusted; Clicky setup commands and repo code were not executed. |
| 2026-05-01 | Codex Chronicle lessons for Cortex | OpenAI Codex Chronicle docs: https://developers.openai.com/codex/memories/chronicle, Codex Memories: https://developers.openai.com/codex/memories, Codex MCP: https://developers.openai.com/codex/mcp, Codex Skills: https://developers.openai.com/codex/skills | 1 | Captured Chronicle's screen-context memory value, consent requirements, storage/rate-limit/prompt-injection risks, and Cortex differentiation | Official OpenAI docs only; external content treated as evidence; no setup commands or Chronicle enablement attempted. |
| 2026-05-14 | Pointer-first AI interaction | Google DeepMind AI Pointer post: https://deepmind.google/blog/ai-pointer/ and official YouTube demo: https://www.youtube.com/watch?v=pZNzfQLgGsA | 1 | Reset Cortex live UX around the pointer as context selector, command surface, output anchor, and receipt trigger | Official Google DeepMind sources only; embedded Google-hosted clips were sampled with `ffprobe`/`ffmpeg`; no external code, install scripts, or non-official analysis used. |
| 2026-05-14 | OpenAI pointer tutor smoke | OpenAI developer docs MCP search results for GPT-5 parameters and Responses API plus endpoint list including `https://api.openai.com/v1/responses` | 1 | Added a cheap-model pointer tutor draft seam using `gpt-5-nano`, minimal reasoning, and `store:false` | Official OpenAI docs tooling only; no API key setup requested, no secret read into docs, and live smoke sends only controlled target facts. |
| 2026-05-14 | Realtime voice pointer UX | OpenAI Realtime overview: https://developers.openai.com/api/docs/guides/realtime, Voice agents: https://developers.openai.com/api/docs/guides/voice-agents, Realtime costs: https://developers.openai.com/api/docs/guides/realtime-costs, `gpt-realtime-2`: https://developers.openai.com/api/docs/models/gpt-realtime-2 | 1 | Implemented `gpt-realtime-2` voice-pointer contracts, gesture routing, cost guard, and sanitized client-secret plan | Official OpenAI docs MCP only; no live microphone, no raw audio, no client secret value, no screen capture, no memory write, and no external action in this slice. |
