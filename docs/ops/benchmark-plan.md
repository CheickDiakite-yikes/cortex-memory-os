...
| `NATIVE-AGENTIC-POINTER-CARD-001` | Native Shadow Clicker bubble can render bounded agentic run copy from the same receipt vocabulary as the dashboard. | Native copy leaks secrets, runs too long, hides route/safety state, starts capture/actions, moves the real cursor, writes memory, retains raw refs, or exports data. |
...
| `OPENAI-TUTOR-SAFE-DRAFT-001` | Pointer-first tutor can ask a cheap OpenAI model for one short draft from controlled target facts with `store:false`. | API key leaks, screenshots/audio/raw refs/memory content are sent, default tests hit the network, or model output can click/type/export/capture/write memory. |
...
| `LIVE-TUTOR-PRODUCT-UX-001` | Live Tutor review side stays product-facing with Chats, Memories, Voice, Safety, and a plain I can / I ask before / I cannot strip. | The side panel regresses into dense logs, hides safety locks, implies silent memory saves, or enables capture/listening/actions. |
...
| `AGENTIC-OS-PLANNER-001` | Pointer-first goals become capability lanes, safe tool routes, approval gates, outcome traces, and reviewed learning proposals. | Planner starts autonomous clicks, typing, exports, raw storage, screen/audio capture, or unreviewed memory writes. |
| `AGENTIC-TURN-ROUTER-001` | A pointer intent becomes a safe route decision, pointer card, approval state, and redacted run receipt. | Router allows click/type/capture/raw refs/external effects, hides approval, or echoes prompt/secret content. |
| `AGENTIC-LIVE-TUTOR-BRIDGE-001` | Controlled Live Tutor turns enter the Agentic OS router without losing target, referent, or review-only memory boundaries. | Bridge treats controlled demo facts as real capture, writes memory, stores raw refs, or drops review gates. |
| `AGENTIC-POINTER-CARD-001` | Pointer-side card copy stays short, target-specific, answer/draft-oriented, and effect-free. | Card becomes a dashboard-only panel, enables actions, jumps away from the pointer, or hides route/safety state. |
| `AGENTIC-RUNTIME-TRACE-001` | Agentic turns create redacted trace receipts with approval state and blocked effects before learning or action. | Trace includes raw payloads, user phrases, assistant responses, source refs, or claims an unapproved effect happened. |
...
| `OPENAI-TUTOR-SAFE-DRAFT-001` | Optional pointer-tutor OpenAI smoke uses ignored `.env.local`, controlled target facts only, `gpt-5-nano`, minimal reasoning, and `store:false`. | The tutor smoke captures real screen/audio, sends raw refs or durable memory content, prints secrets, performs external effects, or defaults to live network calls. |
...
| `REALTIME-VOICE-CONTRACT-001` | `gpt-realtime-2` voice pointer contract uses WebRTC, ephemeral client secrets, text-first output, low reasoning effort, and no default mic. | Browser code gets raw API keys, mic opens by default, screen capture starts, raw audio is retained, or memory writes are enabled. |
| `POINTER-GESTURE-GRAMMAR-001` | Triple click, click and hold, silent hold, and selection gestures have distinct safe meanings. | Gestures execute clicks, start capture/mic directly, or collapse voice/text/silent output into one ambiguous mode. |
| `VOICE-OUTPUT-ROUTER-001` | Voice input routes to silent visual, text chip, spoken brief, or memory review depending on gesture and intent. | Voice output is always on, memory intent becomes durable write, or no-voice-back requests still speak. |
| `REALTIME-COST-GUARD-001` | Cost guard bounds session duration, input/output audio, response count, text tokens, and can force spoken output to text. | Audio output cannot be disabled, session history grows without a cap, or high reasoning effort becomes default. |
| `SYNTHETIC-VOICE-TURN-LOOP-001` | Synthetic transcript-only turns prove gesture -> target -> output -> receipt before live microphone work. | Live microphone, raw audio, memory write, raw ref, or external effect appears in the safe demo. |
| `REALTIME-CLIENT-SECRET-CONTRACT-001` | Realtime client secret plan is server-side, sanitized, and uses `POST /v1/realtime/client_secrets`. | Raw API key or client secret value appears in UI, logs, docs, tests, or benchmark output. |
| `LIVE-TUTOR-VOICE-UX-001` | Live Tutor demo exposes triple-click voice, hold-for-text, and silent action routes while remaining display-only. | The demo starts real voice capture, performs clicks, writes memory, or hides the output mode from receipts. |
| `SELECTION-UX-CONTRACT-001` | Multi-target selection supports "these" without creating an automatic skill or memory. | Selection immediately promotes a workflow, writes memory, or loses target provenance. |
| `DASHBOARD-VOICE-POINTER-PANEL-001` | Dashboard summarizes voice pointer modes simply and safely. | Dashboard shows raw transcripts/audio, API secrets, dense engineering state, or misleading live-mic status. |
| `REALTIME-VOICE-BENCH-DOCS-001` | Architecture and ADR docs preserve official-source Realtime guidance, UX grammar, and cost constraints. | Docs omit the official model path, cost guard, no raw audio, or no live microphone boundary. |
